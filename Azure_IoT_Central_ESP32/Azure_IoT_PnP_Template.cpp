...
#define SAMPLE_ALARM_PROPERTY_NAME "alarm"
...
#define SAMPLE_ALARM_PROPERTY_VALUE "20:02:01"
...
#define TELEMETRY_PROP_NAME_ALARMTIME "alarmTime"
...
int azure_pnp_send_device_info(azure_iot_t* azure_iot, uint32_t request_id, const char* alarm_time)
{
  _az_PRECONDITION_NOT_NULL(azure_iot);
  ...
  result = generate_device_info_payload(
      &azure_iot->iot_hub_client, data_buffer, DATA_BUFFER_SIZE, &length, alarm_time);
...
static char* simulated_get_alarmTime() { return "01:22:23"; }
    ...
    size_t* payload_buffer_length)
{
  ...
  strcpy(time, simulated_get_alarmTime());
  ...
  az_span time_span = az_span_create((uint8_t*)time, strlen(time));
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_ALARMTIME));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding alarmTime property name to telemetry payload.");
  rc = az_json_writer_append_string(&jw, time_span);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding alarmTime property value to telemetry payload.");
    ...
    const char* current_alarm_time)
{
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(SAMPLE_ALARM_PROPERTY_NAME));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding SAMPLE_ALARM_PROPERTY_NAME to payload.");
  // Create an az_span from current_alarm_time
  az_span current_alarm_time_span = az_span_create((uint8_t*)current_alarm_time, strlen(current_alarm_time));
  rc = az_json_writer_append_string(&jw, current_alarm_time_span);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding current_alarm_time to payload.");
